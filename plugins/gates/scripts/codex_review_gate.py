...
#: Переменные, которыми ВЫЗЫВАЮЩИЙ подменяет команду ТРАНСПОРТА. Список живёт рядом с
#: аллоулистом сознательно: `GIT_SSH_COMMAND` имеет приоритет НАД `-c core.sshCommand`
#: (проверено рекордером 2026-08-14), поэтому расширение аллоулиста этими именами молча
#: убило бы закрепление транспорта во ВСЕХ адаптерах сразу. Непересечение проверяется тестом.
_TRANSPORT_ENV_DENY = ("GIT_SSH_COMMAND", "GIT_SSH", "GIT_SSH_VARIANT", "GIT_PROXY_COMMAND",
                       "GIT_CONFIG_PARAMETERS", "GIT_CONFIG_COUNT")

#: Каталог, из которого запускается операция, чей конфиг может содержать ОТНОСИТЕЛЬНЫЕ пути
#: (`ProxyCommand ./proxy`, `core.sshCommand=./x`). Не корень репозитория — иначе они
#: резолвятся в байты проверяемой стороны и исполняются (воспроизведено 2026-08-14). Не дом и
#: не временный каталог: оба населены и записываемы вызывающим. `/` не выбирается никем и не
#: пишется — тот же выбор и по той же причине, что в `_run_baseline_command`.
_NEUTRAL_CWD = os.sep
...
def _trusted_ssh_bin() -> "str | None":
    ...
    Живёт в СЛОЕ, а не у вызывающего: правило «абсолютный, из доверенных каталогов, не внутри
    репозитория» — инвариант слоя, и третий его экземпляр в потребителе означал бы, что
    ужесточение придётся вносить трижды, а пропуск копии будет тихим.

    Зачем вообще: `_GIT_NEUTRALIZE` гасит исполняемые конфиги подстановкой `/nonexistent`. Для
    факультативных (`gpg.program`, `core.editor`) это верно, но `core.sshCommand` при
    SSH-транспорте — САМ транспорт. Гашение подменяло «проверяемая сторона не выберет команду»
    на «команду не запустить вовсе», и pre-push гейт был неисполним для любого SSH-ремоута
    (обнаружено 2026-08-14 при попытке запушить 0.9.1; отказ выглядел как «база не обновлена»).
    Исполняемое, без которого операция невозможна, не отключают — его ПРИКРЕПЛЯЮТ к доверенному
    абсолютному пути, как уже сделано с `--upload-pack` и с самим git."""
    for cand in (Path(d) / "ssh" for d in _TRUSTED_PATH_DIRS):
        try:
            resolved = cand.resolve(strict=True)
        except OSError:
            continue
