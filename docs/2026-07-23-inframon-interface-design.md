...
**Pin одобренной секции `deploy` (R1-F1 + самопроверка).** Изменённая/активированная/
удалённая `baseline_command` может вернуть свежий SHA (вплоть до HEAD) → диапазон сузится/
опустеет → правка команды НЕ ревьюится (self-hiding, напр. staging вместо prod). Сравнение
«base-vs-head» здесь НЕ работает (самопроверка): baseline берётся из САМОЙ команды —
изменённая команда возвращает HEAD → секция HEAD сравнивается сама с собой → атака проходит.
Нужен якорь, НЕЗАВИСИМЫЙ от вывода команды: локальный **pin** `.claude/.deploy-section-pin`
(sha256 нормализованной секции `deploy`). Правила:
- pin отсутствует И секция absent (head) → **bootstrap** (R2-F1: не доказывает legacy —
  на новой машине pin законно отсутствует, а секцию могли уже удалить): сверить секцию на
  baseline из ЛЕГАСИ-источника (env/локальный файл — НЕ из команды, самореференции нет):
  там enabled → **БЛОК** (удаление без перехода), кроме env-перехода; там absent → честный
  legacy, пишется pin-сентинел `disabled` (bootstrap завершён). Остаток (niche): stale-файл,
  указывающий ДО активации секции, скроет удаление — закрывается на стороне inframon.
- pin отсутствует И секция enabled → ПЕРВАЯ активация → **БЛОК**, кроме явного
  `CODEX_DEPLOY_BASELINE` (аудит B7); на allow перехода pin записывается.
- pin есть И hash(секция head_before) == pin → штатно (команда исполняется).
- pin есть И hash ≠ pin (смена/удаление секции) → **БЛОК**, кроме env-перехода; на allow pin
  обновляется (удаление секции → pin снимается).
- pin битый/нечитаемый → как несовпадение → блок, кроме env (fail-closed).
Pin локален и правится руками — это осознанный обход класса «правка маркера» (оператор,
вне охвата); его роль — ловить СЛУЧАЙНОЕ/самоскрывающееся изменение, не злонамеренного
владельца машины (это сторона inframon).
...
**Delete-then-write ПОД ЛОКОМ (R1-F2/R2-F2):** unlink старого + запись нового — под
эксклюзивным локом (класс `findings_lock`): конкурентные allow-прогоны одного head не
interleave'ятся (иначе clean-вердикт параллельного прогона переживал бы провал записи
skip-прогона и маскировал его скипы). Вердикт несёт `run_id` (ts+pid) — задел под
consumer-протокол inframon (сверка «тот ли прогон»), без обязательств сейчас. Провал ЗАПИСИ после успешного unlink →
...
| B9 | секция deploy ≠ pin (первая активация / смена / удаление) | **БЛОК** — переход только через явный `CODEX_DEPLOY_BASELINE`; на allow pin обновляется (R1-F1) |
| B9b | pin битый/нечитаемый | **БЛОК**, кроме env-перехода (fail-closed) |
| B10 | изменённая команда возвращает HEAD (пустой диапазон) | ловится B9: hash(секции) ≠ pin ДО исполнения команды (самопроверка R1-фикса) |
...
| V8 | конкурентные allow-прогоны одного head | unlink+write под локом — не interleave'ятся; вердикт содержит run_id (R2-F2) |
| B11 | pin absent + секция absent (head), на легаси-baseline секция enabled | **БЛОК** — удаление без перехода (R2-F1) |
| B12 | pin absent + секция absent на head И baseline | legacy + pin-сентинел `disabled` (bootstrap) |
...
- **EARS-3b [R3-F1]:** ЛЮБОЙ env-переход, изменяющий pin (первая активация, смена, УДАЛЕНИЕ
  секции, bootstrap-override), SHALL писать аудит-строку с старым и новым состоянием pin —
  включая случай, когда command на head уже отсутствует (удаление). Молчаливое обновление/
  снятие pin ЗАПРЕЩЕНО. (B9, B11)
...
- **EARS-4c [R2-F1]:** IF pin отсутствует И секция absent на head — THEN bootstrap: секция
  SHALL сверяться на легаси-baseline (не из команды); enabled там → блок (кроме env-перехода);
  absent → legacy + запись pin-сентинела `disabled`. (B11, B12)
- **EARS-6b [R2-F2]:** unlink+запись вердикта SHALL выполняться под эксклюзивным локом;
  вердикт SHALL содержать `run_id`. (V8)
- **EARS-4b:** Секция `deploy` (head_before) SHALL сверяться с локальным pin
  (`.claude/.deploy-section-pin`) ДО исполнения команды; несовпадение/первая активация/битый
  pin → БЛОК, кроме явного `CODEX_DEPLOY_BASELINE` (аудируемый переход, обновляющий pin).
...
Юнит: B1–B12 через `resolve_baseline`/`check_reviewed_cli` (command-стабы: echo sha / exit 1 /
