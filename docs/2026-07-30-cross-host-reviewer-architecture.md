...
> ⚠️ **Частично заменена 07.08.2026.** `docs/2026-08-07-host-relative-reviewer-ladder-design.md`
> заменяет: §3.3 (профиль `portable` — теперь обязательная пара Codex+Claude вместо
> одиночного Gemini), инвариант 2 (фиксированный консервативный author set снят: независимость
> обеспечивается присутствием ОБЕИХ возможных авторских семей в панели при union-семантике
> находок) и роль `supplemental` (упразднена — Claude стал blocking внутри пары).
> Инвариант 1 сохранён в сужённом виде: Claude не может быть ЕДИНСТВЕННЫМ blocking-ревьюером.
> Причина замены: в поставке 0.7.0 единственный разрешённый blocking-ревьюер (Gemini) остался
> `candidate`, из-за чего deploy-гейт не пропускал никого ни в одной конфигурации.
> Принятые остатки — M8 (`declared`-аттестация Codex) и R-CERT-PROVENANCE — в `AGENTS.md`.
