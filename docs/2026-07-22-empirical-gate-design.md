...
- **ML-E5 [ОСТАТОК, Фаза 2 — code-R2, исключён из сходимости]** env-ослабление неизменной
  команды: `PATH=/tmp/shims make deploy` подменяет `pytest` шимом, возвращающим 0; `PYTHONPATH`/
  `PYTEST_ADDOPTS`/`MAKEFLAGS` глушат тесты. Команда та же (base==head), гейт зелёный, аудита нет.
  **НЕ чиним (архитектурное):** (1) тест-раннеру environment НУЖЕН (PATH к интерпретатору, venv,
  секреты тестов) — санитизация ломает легитимный прогон (fail-safe пере-блок); (2) кто задаёт
  env на `make deploy`, тот эквивалентен по власти задающему `EMPIRICAL_SKIP` — защита от
  злонамеренного оператора с доступом к деплой-машине вне охвата Фазы 1 (как негеймибельность
  вообще). Герметичный прогон = Фаза 2 (тесты в изолированном образе из отревьюенного коммита,
  тот же immutable-artifact-биндинг, что закрывает reviewed≡deployed). Порог у нас — «поймать
  сломанные тесты + сделать конфиг-обходы аудируемыми», НЕ «остановить оператора-злоумышленника».
